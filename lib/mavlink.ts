...
export abstract class MavLinkProtocol {
   ...
   */
  data<T extends MavLinkData>(
    payload: Buffer,
    clazz: MavLinkDataConstructor<T>
  ): T {
    this.log.trace(
      "Deserializing",
      clazz.MSG_NAME,
...
export class MavLinkProtocolV1 extends MavLinkProtocol {
  ...
  serialize(message: MavLinkData, seq: number): Buffer {
    ...
    const definition: MavLinkDataConstructor<MavLinkData> = <any>(
      message.constructor
    );
    ...
    definition.FIELDS.forEach((field) => {
      ...
      if (!serialize)
        throw new Error(
          `Unknown field type ${field.type}: serializer not found`
        );
...
export class MavLinkProtocolV2 extends MavLinkProtocol {
  ...
  private calculateTruncatedPayloadLength(buffer: Buffer): number {
    ...
    for (
      let i = buffer.length - MavLinkProtocol.CHECKSUM_LENGTH - 1;
...
export class MavLinkPacket {
  ...
  debug() {
    return (
      ...
      // @ts-ignore
      `magic: ${MSG_ID_MAGIC_NUMBER[this.header.msgid]} (${hex(
        // @ts-ignore
        MSG_ID_MAGIC_NUMBER[this.header.msgid]
      )}), ` +
      `crc: ${hex(this.crc, 4)}` +
      // @ts-ignore
...
export class MavLinkPacketSplitter extends Transform {
  ...
  private readPacketLength(
    buffer: Buffer,
    Protocol: MavLinkProtocolConstructor
  ) {
    // check if the current buffer contains the entire message
  ...
  private validatePacket(buffer: Buffer, Protocol: MavLinkProtocolConstructor) {
    ...
    if (magic) {
      ...
      if (crc === crc2) {
        ...
        // CRC mismatch
        const message = [
          `CRC error; expected: ${crc2} (${hex(crc2, 4)}), got ${crc} (${hex(
            crc,
            4
          )});`,
      ...
      this.log.debug(
        `Unknown message with id ${header.msgid} (magic number not found) - skipping`
      );
...
export class MavLinkPacketParser extends Transform {
  ...
  _transform(
    { buffer = Buffer.from([]), timestamp = null, ...rest } = {},
    encoding: string,
    ...
    const crc = protocol.crc(buffer);
    const signature =
      protocol instanceof MavLinkProtocolV2
...
export async function send(
  stream: Writable,
  msg: MavLinkData,
  protocol: MavLinkProtocol = new MavLinkProtocolV1()
) {
...
export async function sendSigned(
  ...
  key: Buffer,
  linkId: uint8_t = 1,
  sysid: uint8_t = MavLinkProtocol.SYS_ID,
  compid: uint8_t = MavLinkProtocol.COMP_ID,
  timestamp = Date.now()
) {
  return new Promise((resolve, reject) => {
